  ...
  CREATE_NEW_PROJECT: {
    action: createUILockAction(
      async (context, { confirm }: { confirm?: boolean }) => {
        ...
        context.mutations.SET_SAVED_LAST_COMMAND_UNIX_MILLISEC(null);
          ...
          const parsedProjectData = await migrateProjectFileObject(
            projectData,
            {
              fetchMoraData: (payload) =>
                context.actions.FETCH_MORA_DATA(payload),
              characterInfos,
            },
          );
      ...
      async (context, { overwrite }: { overwrite?: boolean }) => {
        ...
        try {
          ...
          await window.backend
            ...
            .then(getValueOrThrow);
          context.mutations.SET_PROJECT_FILEPATH({ filePath });
          context.mutations.SET_SAVED_LAST_COMMAND_UNIX_MILLISEC(
